...
This script is used to train a model on the ImageNet dataset using the Discrete Diffusion Classification Modeling framework
proposed in the paper "Advancing Image Classification with Discrete Diffusion Classification Modeling" by Omer Belhasin.
The training script was adapted from the ImageNet Training Script in the timm library.
"""
import sys
import argparse
import copy
...
import didicm
from didicm.utils import data_utils
...
group.add_argument('--data-ratio', default=None, type=float,
                   help='Data ratio for training dataset (default: None)')
...
group.add_argument('--auto-resume', action='store_true', default=False,
                   help='Automatically resume training from the latest checkpoint in the output directory.')
...
group.add_argument('--wandb-tags', default=[], type=str, nargs='+',
                   help='wandb tags')
group.add_argument('--wandb-resume-id', default=None, type=str, metavar='ID',
...
group.add_argument('--diffusion-enabled', action='store_true', default=False,
                   help='Enable diffusion')
group.add_argument('--diffusion-sampler', type=str, default='cp',
                   help='Sampler for diffusion')
group.add_argument('--diffusion-sampler-kwargs', nargs='*', default={}, action=utils.ParseKwargs,
                   help='Keyword arguments for diffusion sampler')
group.add_argument('--diffusion-noise', type=str, default='loglinear',
                   help='Noise type for diffusion')
group.add_argument('--diffusion-steps', type=int, default=8,
                   help='Number of diffusion steps')
group.add_argument('--diffusion-eps', type=float, default=1e-5,
                   help='Epsilon for diffusion sampling')
...
def main():
    ...
    if args.auto_resume and not args.resume:
        ...
        last_checkpoint_path = os.path.join(output_dir, 'last.pth.tar')
        if os.path.exists(last_checkpoint_path):
            args.resume = last_checkpoint_path
        else:
            if utils.is_primary(args):
                _logger.info(f'No checkpoint found in {output_dir}, starting from scratch.')
    ...
    if args.data_ratio:
        dataset_train = data_utils.change_dataset_to_subset_for_data_ratio(dataset_train, args.data_ratio)
        if utils.is_primary(args):
            _logger.info(f'Using {args.data_ratio} of training data')
    else:
        if utils.is_primary(args):
            _logger.info('Using full training data')
    ...
    if args.diffusion_enabled:
        mixup_active = args.mixup > 0 or args.cutmix > 0. or args.cutmix_minmax is not None
        diffusion_loss_fn = didicm.loss.get_loss_fn(
            num_classes=args.num_classes,
            noise_type=args.diffusion_noise,
            sampling_eps=args.diffusion_eps,
            amp_autocast=amp_autocast,
            mixup_active=mixup_active
        )
        diffusion_sampler = didicm.score_sampling.get_sampler(
            sampler=args.diffusion_sampler,
            num_classes=args.num_classes,
            noise_type=args.diffusion_noise,
            steps=args.diffusion_steps,
            eps=args.diffusion_eps,
            **args.diffusion_sampler_kwargs
        )
        if utils.is_primary(args):
            _logger.info(f'Using diffusion with loss: {diffusion_loss_fn.__class__.__name__}, sampler: {diffusion_sampler.__class__.__name__}')

    else:
        if utils.is_primary(args):
            _logger.info('No diffusion enabled')

    # setup loss function
    if args.diffusion_enabled:
        train_loss_fn = diffusion_loss_fn
    else:
        if args.jsd_loss:
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(loader):
        input = batch[0]
        target = batch[1]
        noise_level = batch[2] if len(batch) > 2 else None
        ...
        def _forward():
            with amp_autocast():
                if args.diffusion_enabled:
                    _loss = loss_fn(model, input, target, t=noise_level)
                else:
                    output = model(input)
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                ...
                if args.diffusion_enabled:
                    ...
                    output = diffusion_sampler.run(model, input)

                    if args.diffusion_enabled and reduce_factor > 1:
                        raise NotImplementedError("Reduction factor > 1 not implemented")
